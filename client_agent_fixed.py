...
class SignageClient:
        ...
        try:
            ...
            if env.get('DISPLAY') and not env.get('XAUTHORITY'):
                ...
                if os.path.exists(xauth_path):
                    ...
                    self.logger.debug(f"Added XAUTHORITY: {xauth_path}")
            ...
            # Wait for the specified duration (or default)
            if duration:
                time.sleep(duration)
                self.stop_current_media()
                return True
    ...
    def play_playlist(self):
        ...
        if self.current_media_index >= len(items):
            if self.current_playlist.get('loop'):
                self.logger.info("Playlist completed, looping")
                self.current_media_index = 0
            else:
