...
- October 22, 2025. PWA support for iOS/Android app installation: Implemented complete Progressive Web App functionality with web app manifest, app icons in multiple sizes (512x512, 192x192, 180x180, 32x32, 16x16), favicon.ico, and iOS-specific meta tags. Users can now install DisplayHQ as a standalone app on iOS and Android devices. App icons feature DisplayHQ monitor/tablet branding with dark background and cyan accents. Manifest configured with landscape orientation and /control as start URL for optimal tablet experience.
- October 22, 2025. Natural device sorting: Implemented numerical sorting for device lists on both /devices and /control pages. Devices now sort in human-friendly order (e.g., "Lobby TV 2" before "Lobby TV 10" instead of lexicographic "Lobby TV 10" before "Lobby TV 2"). Uses natural_sort_key helper function that splits device names into numeric and text segments for intelligent ordering.
- November 4, 2025. VNC remote access script: Created standalone install_vnc.sh script for optional VNC server installation on client devices. Script installs x11vnc, sets password protection, creates systemd service for auto-start, and provides connection details. Not installed automatically - must be manually deployed via curl or download. Enables remote desktop access for troubleshooting and management without TeamViewer.
- November 19, 2025. HLS stream health monitoring and auto-restart: Implemented comprehensive stream health monitoring system (CLIENT_VERSION 2.3.8) that detects frozen HLS streams and automatically restarts VLC. Uses VLC's RC (remote control) interface via Unix socket to monitor demux progress every 10 seconds. Automatically restarts playback if stream stalls for more than 30 seconds or after 3 consecutive monitoring errors. Features 60-second cooldown period between restarts to prevent flapping. Logs all stall events and restarts to server for troubleshooting. Only activates when playing HLS streams (m3u8 files), leaving regular media playback unaffected. Includes defensive fallbacks to ensure stall detection works even if stream freezes before first demux progress.
