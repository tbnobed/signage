...
- October 21, 2025. VNC remote access (CLIENT_VERSION = "2.4.0"): Added automatic VNC server enabling for Raspberry Pi devices during setup. Setup script detects Raspberry Pi OS and uses raspi-config to enable VNC, or installs TigerVNC server on Ubuntu. Provides remote desktop access for easier device management and troubleshooting without requiring physical access to the display.
- October 21, 2025. Wayland display fix (CLIENT_VERSION = "2.4.2"): Fixed critical VLC display issue on Wayland-based systems (Raspberry Pi OS, Ubuntu Wayland). Removed forced --vout x11 parameter that prevented VLC from displaying on Wayland compositors. VLC now auto-detects the appropriate video output (X11 or Wayland) for maximum compatibility. Enhanced systemd service with DISPLAY, XDG_RUNTIME_DIR, and WAYLAND_DISPLAY environment variables for proper display access across all desktop environments.
- October 21, 2025. Universal media codec support (CLIENT_VERSION = "2.4.4"): Optimized mpv for Raspberry Pi to handle all media codecs without conversion. Configured multi-threaded software decoding with 4 CPU cores (--vd-lavc-threads=4), intelligent frame dropping (--framedrop=vo), and GPU sync optimizations for smooth playback of MJPEG, H.264, and all other video formats. Fixed setup_client.py syntax warning in GDM configuration.
- October 21, 2025. Raspberry Pi installer and playback fixes (CLIENT_VERSION = "2.4.5"): Fixed setup_client.py to skip GNOME/GDM configuration on Raspberry Pi OS (uses LightDM instead). Wrapped all gsettings commands in GNOME detection check to prevent installer crashes on non-GNOME systems. Fixed mpv GPU context to use valid wayland,x11egl options (removed deprecated 'x11'). Added video scaling flags (--video-unscaled=no --panscan=1.0) to properly scale 4K content to 1080p displays. Installer now completes successfully on Raspberry Pi OS without errors.
